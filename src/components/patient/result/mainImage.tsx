...
// export default function MainImage({ images, imageIds, setCurrentImageId }: IProps) {
...
//   useEffect(() => {
//     if (images && imageIds) {
//       const normalIds = images.map((_, index) => imageIds[index])
//       setCurrentImageId(normalIds[0] || null)
//     }
//   }, [setCurrentImageId])

//   useEffect(() => {
//     if (images) {
//       if (currentType === 'UFI' && images[currentImageIndex]) {
//         setCurrentImageId(images[currentImageIndex])
//       } else if (currentType === 'CFI' && images[currentImageIndex]) {
//         setCurrentImageId(images[currentImageIndex])
//       }
//     }
//   }, [currentImageIndex, images, setCurrentImageId, currentType])
...
//                     <img
//                       src={images?.[currentImageIndex] || defaultAvatar}
//                       alt=''
...
//           </TransformWrapper>
//         </div>
//         <div className='mt-4 flex-center-y relative'>
...
//               className={`${currentType === 'UFI' ? 'bg-slate-500 text-white' : 'bg-slate-300'} px-4 py-10 h-full rounded-l-lg `}
//             >
//               UFI
//             </button>
...
//               className={`${currentType === 'CFI' ? 'bg-slate-500 text-white' : 'bg-slate-300'} px-4 py-10 h-full rounded-r-lg`}
//             >
//               CFI
//             </button>
//           </div>
...
export default function MainImage({ grayImages, normalImages, imageIds, id, setCurrentImageId }: IProps) {
  const [currentImageIndex, setCurrentImageIndex] = useState<number>(0)
  ...
  return (
    <section id='main-image' className='h-full gap-2 md:flex'>
      ...
      <div className='flex w-full gap-2 mb-4 overflow-auto md:block md:w-40 md:mb-0'>
        ...
        {currentType === 'UFI' &&
          uploadedNormalImages.map((file, index) => (
            <div key={index} className='flex-shrink-0 image-container w-28 aspect-square'>
              <img
        ...
        {currentType === 'CFI' &&
          uploadedGrayImages.map((file, index) => (
            <div key={index} className='flex-shrink-0 image-container w-28 aspect-square'>
              <img
      ...
      <div className='w-full'>
        ...
        <div className='mt-4 flex-center-x relative h-12'>
          <div className='absolute right-0 border-2 inset-y-0 border border-slate-400 rounded-xl flex-center-y overflow-hidden'>
            <button
              onClick={() => handleSwitImgType('UFI')}
