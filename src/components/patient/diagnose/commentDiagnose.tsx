...
const CommentDiagnose: React.FC<IProps> = ({ data, savedValues, currentImageId }) => {
  ...
  return (
    <section id='image-result' className='pt-2 bg-white rounded-xl shadow-pop'>
      <div className='justify-between gap-5 px-4 pb-2 mb-1 border-b-2 border-slate-400 flex-center-y'>
        ...
        <button
          onClick={() => {
            ...
            // handleSaveFeedback(); // Save feedback when the button is clicked
          }}
      ...
      {confirmClicked ? (
        <p className='text-center mt-2 text-gray-500'>Feedback confirmed</p>
      ) : (
        <div className='flex justify-center mt-2'>
          <button
            onClick={handleConfirm}
            className='px-4 py-2 font-bold text-white rounded-lg bg-green-500 hover:bg-green-600'
          >
            CONFIRM
          </button>
        </div>
...
export default CommentDiagnose;
